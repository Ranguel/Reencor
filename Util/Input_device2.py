...
from Util.Common_functions import RoundSign, get_command, object_image
...
class InputDevice():
    ...
    def assign_object(self, control_object):
        self.control_object = control_object
        self.control_object.input_device = self
    ...
    def get_press(self, raw_input):
        ...
        dpad_last = str([['8', '2', '5'], ['9', '3', '6'], ['7', '1', '4']][self.last_input[0][0]][self.last_input[0][1]-1])
        commands = []

        for index, move in enumerate(self.sequence_commands):
            if (dpad is self.sequence_commands[index]["sequence"][self.sequence_index[index]] and dpad_last is self.sequence_commands[index]["sequence"][self.sequence_index[index]-1]):
                self.sequence_index[index] = self.sequence_index[index] + 1
                if self.sequence_index[index]>=len(self.sequence_commands[index]["sequence"]):
        ...
        for index in range(1, 7):
            ...
            if 0<self.press_charge[index]<6 and pressed_buttons:
                pressed_buttons.append("p_b"+str(index))
            if self.press_charge[index] > 120:
                holded_buttons.append("charge_b" + str(index))

            
        for index,value in enumerate(self.sequence_index):
            if value == -1:
                for button in pressed_buttons + released_buttons + holded_buttons:
                    self.sequence_index[index] = 1
                    commands.append(self.sequence_commands[index]["command"]+" + "+button)
        ...
        self.current_input = [dpad, dpad_last] + pressed_buttons + released_buttons + holded_buttons + commands
        ...
        if raw_input != self.last_input:
            ...
            try:
                self.control_object.input_interruption = True
                self.control_object.current_command += list(self.current_input)
                get_command(self.control_object, self.control_object.current_command)
...
class InputListener(threading.Thread):
    def __init__(self, game, input_device_list: list = [InputDevice, InputDevice]):
        super().__init__()
        self.game, self.device_list = game, input_device_list
        self.daemon = True
